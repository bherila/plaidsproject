...
class MaintenanceMode extends MaintenanceMode_SWGPluginFramework {
	...
	function ApplyMaintenanceMode() {
		...
		if ($this->g_opt['mamo_activate'] == 'on') {
			...
			if( (!$this->current_user_can_access_on_maintenance()) 
					&& ($this->g_opt['mamo_noaccesstobackend'] == '1') && (!strstr($_SERVER['PHP_SELF'], 'wp-login.php'))
					// For some reason current_user_can_access_on_maintenance() returns FALSE when site is loaded by MediaUploader or some plugins so we add this piece of code qulick & dirty...					
			...
			if(	!strstr($_SERVER['PHP_SELF'], 'feed/') 
					...
					&& !strstr($_SERVER['PHP_SELF'], '/plugins/') // Included plugins directory so that currently enabled plugins work while in maintenance mode.
					&& !is_admin()  
